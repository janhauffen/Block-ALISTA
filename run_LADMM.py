...
for k in range(0, len(SNR_list)):
    ...
    layers = network.build_LADMM(prob,T=T,initial_lambda=0.1, initial_rho=0.1)
    start = time.time()
    ...
    sess = train.do_training(training_stages,prob,'trainings/LADMM_T'+str(T)+'_SNRdB_'+str(SNR)+'batch'+str(MC)+'.npz')
    """# Evaluating"""
