...
from __future__ import print_function
import numpy as np
SNR_list=[np.infty, 40, 30, 20, 10, 5, 0]
for k in range(0, len(SNR_list)):
    ...
    #sess = train.do_training(training_stages,prob,'trainings/BALISTA_v4_block_Gauss_giidT'+str(T)+'Thermo6-JanWithSNR_dB'+str(SNR)+'.npz')
    """# Evaluating"""
    ...
    #plt.savefig('images/BISTAThm6T'+str(T)+'WithSNR_dB'+str(SNR)+'.png')
    #plt.close()
    #plt.plot(gam)
    #plt.title('sigma = ' + str(sigma))
    #plt.savefig('images/BISTAGammasT'+str(T)+'WithSNR_dB'+str(SNR)+'.png')
    #plt.close()
    #plt.plot(10*np.log10(nmse_dbLISTAMean), label = 'nmse_db mean')
    #plt.legend()
    #plt.title('sigma = ' + str(sigma))
    #plt.savefig('images/BISTAMeanNmseDBT'+str(T)+'WithSNR_dB'+str(SNR)+'.png')
    #plt.close()
    #plt.plot(lam, label = 'lambdas')
    #plt.legend()
    #plt.title('sigma = ' + str(sigma))
    #plt.savefig('images/BISTAlambdas'+str(T)+'WithSNR_dB'+str(SNR)+'.png')
    #plt.close()
    #pdb.set_trace())
    ...
    sio.savemat('mat/BISTAnmseSNR'+str(SNR)+'dB.mat', {'nmseMean': nmse_dbLISTAMean, 'nmse': nmse_dbLISTA})
