...
for k in range(0, len(SNR_list)):
    ...
    layers = network.build_LBISTA(prob,T=T,initial_lambda=0.1, untied=False)
    ...
    #plt.savefig('images/LBISTAGammasT'+str(T)+'WithSNR_dB'+str(SNR)+'.png')
    #plt.close()
    #plt.plot(10*np.log10(nmse_dbLISTAMean), label = 'nmse_db mean')
    #plt.legend()
    #plt.title('sigma = ' + str(sigma))
    #plt.savefig('images/LBISTAMeanNmseDBT'+str(T)+'WithSNR_dB'+str(SNR)+'.png')
    #plt.close()
    #plt.plot(lam, label = 'lambdas')
    #plt.legend()
    #plt.title('sigma = ' + str(sigma))
    #plt.savefig('images/LBISTAlambdas'+str(T)+'WithSNR_dB'+str(SNR)+'.png')
    #plt.close()
    # sio.savemat('mat/0tiedLBISTAnmseSNR'+str(SNR)+'dB.mat', {'nmseMean': nmse_dbLISTAMean, 'nmse': nmse_dbLISTA})
    sio.savemat('mat/tiedLBISTAalpha.mat', {'al': lam})
