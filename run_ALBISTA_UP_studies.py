...
for k in range(0, len(SNR_list)):
    ...
    layers, W = network.build_BALISTA_v4(prob,T=T,initial_lambda=0.5, initial_gamma=0.1, untied=False)
    ...
    sess = train.do_training(training_stages,prob,'trainings/batch250_sparse_case_D_ALBISTA_UP_v4_block_Gauss_giidT'+str(T)+'Thermo6-JanWithSNR_dB'+str(SNR)+'.npz')
    """# Evaluating"""
    # sparsemat = sio.loadmat('blocktest.mat')
    # y = sparsemat.get('y')
    # x = sparsemat.get('x')
    y,x = prob(sess)
    sio.savemat('mat/sparse_case_D_blocktestSNR' + str(SNR) + '.mat', {'x': x, 'y': y})
    # sparsemat = sio.loadmat('mat/D_blocktestSNR' + str(SNR) + '.mat')
    # y = sparsemat.get('y')'ae][
    # x = sparsemat.get('x')
    MC = x.shape[-1]
    t=0
    ...
    #axes[1].plot(x[:,10])
    #pdb.set_trace()
    sio.savemat('mat/sparse_case_D_ALBISTA_UPnmseSNR' + str(SNR) + 'dB.mat', {'nmseMean': nmse_dbLISTAMean, 'nmse': nmse_dbLISTA, 'l21norm': l2norm})
    sio.savemat('mat/sparse_case_D_ALBISTA_Upalpha.mat', {'al': lam, 'gam': gam})
