...
for k in range(0, len(SNR_list)):
    ...
    layers = network.build_TiBLISTA(prob,T=T,initial_lambda=0.1, initial_gamma=0.1, untied=False)
    ...
    # x = sparsemat.get('x')
    # y,x = prob(sess)
    # sio.savemat('mat/0blocktestSNR'+str(SNR)+'.mat', {'x': x, 'y': y})
    ...
    for name, xhat_, var_list in layers:
        if not name == 'Linear':
            lam[k-1], gam[k-1] = sess.run([layers[k][2][0], layers[k][2][1]])
            k = k+1
    ...
    #plt.savefig('images/TiLBISTAThm6T'+str(T)+'WithSNR_dB'+str(SNR)+'.png')
    #plt.close()
    #plt.plot(gam)
    #plt.title('sigma = ' + str(sigma))
    #plt.savefig('images/TiLBISTAGammasT'+str(T)+'WithSNR_dB'+str(SNR)+'.png')
    #plt.close()
    #plt.plot(10*np.log10(nmse_dbLISTAMean), label = 'nmse_db mean')
    #plt.legend()
    #plt.title('sigma = ' + str(sigma))
    #plt.savefig('images/TiLBISTAMeanNmseDBT'+str(T)+'WithSNR_dB'+str(SNR)+'.png')
    #plt.close()
    #plt.plot(lam, label = 'lambdas')
    #plt.legend()
    #plt.title('sigma = ' + str(sigma))
    #plt.savefig('images/TiLBISTAlambdas'+str(T)+'WithSNR_dB'+str(SNR)+'.png')
    #plt.close()
    sio.savemat('mat/normalized_D_TiLBISTAnmseSNR'+str(SNR)+'dB.mat', {'nmseMean': nmse_dbLISTAMean, 'nmse': nmse_dbLISTA})
    sio.savemat('mat/normalized_D_TiLBISTAalpha.mat', {'al': lam, 'gam': gam})
